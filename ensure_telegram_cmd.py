...
# A) Ensure cmd_telegram exists (append if missing)
...
# B) Register the subparser near the extras block; if not found, append at end of parser setup
if 'add_parser("telegram"' not in s:
    # Prefer inserting before the "---- end extras ----" marker if present
    inserted = False
    s2 = re.sub(
        r"(\#\s*----\s*end extras\s*----)",
        r"""
...
\g<1>""",
    ...
    # Fallback: if marker missing, try to insert after publish registration
    if not inserted and 'add_parser("publish"' in s:
        s = re.sub(
            r'(pb\.set_defaults\(func=cmd_publish\)\s*)',
            r'\1' + r"""
    # telegram
    tg = sub.add_parser("telegram", help="Send social.csv images+captions to Telegram")
    tg.add_argument("--date", default=None, help="YYYY-MM-DD or 'latest'")
...
if s != orig:
    ...
    print("telegram subcommand installed/verified.")
else:
