...
# --- 1) Ensure Body is imported with FastAPI (idempotent) ---
if ($t -notmatch 'from fastapi import .*Body') {
  $t = $t -replace 'from fastapi import FastAPI, Request, Response\b',
                   'from fastapi import FastAPI, Request, Response, Body'
}

# --- 2) Inject helpers (only once) before tests marker or append ---
...
'@
  ...
  if ($pos -ge 0) { $t = $t.Insert($pos, $helpers + "`r`n") }
  else { $t = $t + "`r`n" + $helpers + "`r`n" }
}

# --- 3) Add FastAPI endpoints (only once) ---
...
'@
  ...
  if ($retPos -ge 0) { $t = $t.Insert($retPos, $apiBlock) }
}

# --- 4) Add fallback server routes in do_POST (only once) ---
...
$fallback = @'
            ...
            return
'@
  $needle = 'self._send(404, "text/plain; charset=utf-8", b"Not found")'
  $nfPos = $t.IndexOf($needle)
  if ($nfPos -ge 0) { $t = $t.Insert($nfPos, $fallback) }
}

# --- 5) Add UI buttons after the "Open runs" button (only once) ---
$anchorRuns = @'
...
if ($t.Contains($anchorRuns) -and -not $t.Contains('Zip outbox')) {
  ...
  if ($idx -ge 0) { $t = $t.Insert($idx + $anchorRuns.Length, "`r`n" + $btnInsert) }
}

# --- 6) Add JS functions before the first </script> (only once) ---
...
'@
  ...
  if ($first -ge 0) { $t = $t.Insert($first, "`r`n" + $js + "`r`n") }
}
