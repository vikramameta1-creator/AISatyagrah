...
# DO NOT use $Host (built-in). Use $ADDR.
$ADDR = "127.0.0.1"
$PORT = 9000
$API  = "http://${ADDR}:$PORT"

$HARDRESET = $false   # set $true to archive old runs & clear exports\latest

# Prefer venv python if available
$VENV_PY = Join-Path $ROOT ".venv\Scripts\python.exe"
if (Test-Path $VENV_PY) { $PY = $VENV_PY } else { $PY = "python" }
...
function Stop-Uvicorn {
  Get-Process -ErrorAction SilentlyContinue |
    Where-Object { ($_.Name -match 'python|uvicorn') -and $_.Path -and ($_.Path -like "*AISatyagrah*") } |
...
function Start-Uvicorn {
  ...
  $args = @(
    ...
    "--factory","--host",$ADDR,"--port",$PORT,"--reload"
  )
...
function PS5-MultipartUpload([string]$Uri, [string]$FilePath, $Headers=@{}) {
  Add-Type -AssemblyName System.Net.Http
  $hc = [System.Net.Http.HttpClient]::new()
  foreach ($k in $Headers.Keys) {
    $hc.DefaultRequestHeaders.Remove($k) | Out-Null
    $hc.DefaultRequestHeaders.Add($k,[string]$Headers[$k])
  }
  ...
  try {
    ...
    $mp.Add($sc)
    try { $resp = $hc.PostAsync($Uri,$mp).Result } catch { return @{ ok=$false; error="Connect failed: $($_.Exception.Message)"; uri=$Uri } }
  ...
  } finally { $fs.Dispose(); $mp.Dispose(); $hc.Dispose() }
}

function Backup-Now {
  New-Item -ItemType Directory -Force -Path $ARCH | Out-Null
  $ts  = Get-Date -Format "yyyyMMdd_HHmmss"
  $tmp = Join-Path $env:TEMP ("ais_backup_"+$ts)
...
(function(){
  ...
  function api(p,o){const h=o&&o.headers?o.headers:{};const t=tok();if(t)h["x-auth"]=t;return fetch(p,{method:o&&o.method?o.method:"GET",headers:h})}
  ...
  $("#tb-search-btn").onclick=()=>alert("Use list filters.");
...
(function(){
  ...
  function render(items){
    ...
    for(const it of items){
      ...
      c.innerHTML=`<div class="row">
        ...
        <p>${it.snippet||""}</p><div class="mono">${(it.hashtags||"").replace(/#/g,"#<wbr>")}</div>`;
      root.appendChild(c);
    }
  }
  async function load(){
  ...
  $("#fromcsv").onclick=()=>alert("Use reset_newsroom_v3.ps1 helper PS5-MultipartUpload to POST a CSV.");
  $("#run").onclick = async ()=>{
...
try {
  ...
  $cnt = if ($j -and $j.items) { ($j.items | Measure-Object).Count } else { 0 }
  Write-Host ("API OK. Items: {0}" -f $cnt) -ForegroundColor Green
} catch {
  Write-Warning ("Smoke test failed: {0}" -f $_.Exception.Message)
}

Write-Host ""
Write-Host "CSV upload helper usage:" -ForegroundColor Cyan
Write-Host "  \$csv=`"D:\AISatyagrah\imports\newsroom.csv`""
Write-Host "  \$uri=`"$API/api/newsroom/import_csv?date=2025-10-08&platform=telegram`""
Write-Host "  PS5-MultipartUpload -Uri \$uri -FilePath \$csv -Headers @{ `"x-auth`" = `"mysupersecrettoken`" }"
Write-Host ""
Write-Host ("Open UI: {0}/ui/newsroom" -f $API) -ForegroundColor Green
